...
void c_ui_gameselector::think()
{
	Vec2 drawPos = m_parent->get_child_draw_pos() + m_pos;
	Vec2 contentMax(m_size.x, m_size.y + ((m_games.size() - 1) * m_size.y + 2));
	...
	if (is_mouse_over())
	{
		int wheel_delta = g_ui.get_mouse_wheel();

		if (wheel_delta != 0)
		{
			m_scroll_offset -= wheel_delta * m_size.y;

			if (m_scroll_offset < 0) m_scroll_offset = 0;

			int max_scroll = (m_games.size() * m_size.y + 2) - m_height;
...
void c_ui_gameselector::draw()
{
	...
	for (int i = 0; i < m_games.size(); i++) {
		...
		m_games[i].game_icon->begin(D3DXSPRITE_ALPHABLEND);
		m_games[i].game_icon->draw(itemSelectedMin + Vec2(4, 4), c_color(255, 255, 255, 180));
		m_games[i].game_icon->end();

		g_font[Tahoma12px].draw(m_games[i].game_name, itemSelectedMin.x + 35, itemSelectedMin.y + 3,
			TEXT_OUTLINE, (isHovered || isSelected) ? c_color(153, 195, 255) : c_color(200, 200, 200));
	}
...
bool c_ui_gameselector::is_mouse_over()
{
	Vec2 drawPos = m_parent->get_child_draw_pos() + m_pos;
	Vec2 contentMax(m_size.x, m_size.y + ((m_games.size() - 1) * m_size.y + 2));
